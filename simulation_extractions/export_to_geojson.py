...
def __process_one_time_bucket_weight(path_weight_json: Path,
    ...
    for __t_node_id in seq_ids_node:
        ...
        # _point_obj = geojson.Point(_t_position)
        # _point_obj_f = geojson.Feature(geometry=_point_obj, properties={'lane_id': __t_node_id[0], 'weights': __t_node_id[1]})
        
        _r = RecordObject(
            latitude=_t_position[0],
            longitude=_t_position[1],
            lane_id=__t_node_id[0],
            weight_value=__t_node_id[1])
        seq_positions.append(_r)
    # end for
    ...
    for __t_lane_id in seq_ids_lanes:
        ...
        if _t_position is None:
            continue
        # end if    
        # _polygon_obj = geojson.MultiPolygon(_t_position)
        # _polygon_obj_f = geojson.Feature(_polygon_obj, properties={'lane_id': __t_lane_id[0], 'weights': __t_lane_id[1]})
        # Adding polygon object
        for _position_polygon in _t_position:
            _r = RecordObject(
                latitude=_position_polygon[0],
                longitude=_position_polygon[1],
                lane_id=__lane_id,
                weight_value=__t_lane_id[1])
            seq_positions.append(_r)
    # end for
    
    f_out = path_out_csv.open('w')
    f_out.write('latitude,longitude,lane_id,weight_value\n')
    for __record in seq_positions:
        f_out.write(__record.to_csv_record() + '\n')
    # end for
    # with path_geo_json.open('w') as f:
    #     geojson.dump(f, seq_positions, sort_keys=True)
    f_out.close()
    logger.debug(f'GeoJSON file written to {path_out_csv}')
...
def main(path_sumo_net_xml: Path, 
    ...
    path_dir_csv.mkdir(parents=True, exist_ok=True)
    ...
    for __path_weight_json in seq_list_road_list_json:
        ...
        _path_geo_csv_out = path_dir_csv / f'{__path_weight_json.stem}.csv'
        __process_one_time_bucket_weight(
            path_out_csv=_path_geo_csv_out,
...
if __name__ == "__main__":
    ...
    opt.add_argument("--path_dir_csv", type=Path, required=True)
    __args = opt.parse_args()
    
    main(
        path_sumo_net_xml=__args.path_sumo_net_xml,
        path_dir_csv=__args.path_dir_csv,
