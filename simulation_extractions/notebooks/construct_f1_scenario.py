...
iter_bus_stop = root.findall('busStop')

for elem in iter_bus_stop:
...
iter_root_node = root.findall('route')

for elem in iter_root_node:
    # do something with each element here
    element_name: str = elem.tag
...
iter_parking_area_node = root.findall('parkingArea')

for elem in iter_parking_area_node:
    # do something with each element here
    element_name: str = elem.tag
...
iter_parking_area_node = root.findall('rerouter')
for elem in iter_parking_area_node:
    # updating rerouter
...
iter_vehicle_node = root.findall('vehicle')

for elem in iter_vehicle_node:
    # do something with each element here
    element_name: str = elem.tag
...
iter_person_node = root.findall('person')

for elem in iter_person_node:
    # do something with each element here
    element_name: str = elem.tag
    # when True, then I delete the agent.
    is_delete_agent = False
    # process for person tag
    _id_pedestrian = elem.attrib['id']
    ...
    # If this pedestrian requires a ride that passes through the prohibited edges, I delete the pedestrian.
